...
with gr.Blocks(theme=env.theme, title="Semi-Auto-NovelAI-to-Pixiv") as demo:
    gr.Markdown(
        "# [Semi-Auto-NovelAI-to-Pixiv](https://github.com/zhulinyv/Semi-Auto-NovelAI-to-Pixiv) | 半自动 NovelAI 上传 Pixiv"
    )
    with gr.Tab("文生图"):
        gr.Markdown(
            "> 等同于使用 NovelAI 官网, 支持你喜欢的画风串. 如果未返回图片, 多半是 500(服务器负载过高), 429(请求过快), 443(节点不稳定), 反正不是我的问题 ヾ(≧▽≦*)o"
        )
        with gr.Column():
            with gr.Column(scale=3):
                positive = gr.Textbox(
                    value="[suimya, muririn], artist:ciloranko,[artist:sho_(sho_lwlw)],[[tianliang duohe fangdongye]], [eip (pepai)], [rukako], [[[memmo]]], [[[[[hoshi (snacherubi)]]]]], year 2023, 1girl, cute, loli,",
                    lines=2,
                    label="正面提示词",
                )
                with gr.Row():
                    negative = gr.Textbox(
                        value="lowres, {bad}, error, fewer, extra, missing, worst quality, jpeg artifacts, bad quality, watermark, unfinished, displeasing, chromatic aberration, signature, extra digits, artistic error, username, scan, [abstract], mosaic censoring, bar censor, censored, {{{{{chibi,doll}}}}}, silhouette,",
                        lines=2,
                        label="负面提示词",
                        scale=3,
                    )
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    noise_schedule = gr.Radio(
                        ["native", "karras", "exponential", "polyexponential"], value="native", label="噪声计划表"
                    )
            ...
            with gr.Column():
                with gr.Row():
                    ...
                    realcugan_model = gr.Radio(
                        ["models-se", "models-pro", "models-nose"], value="models-se", label="超分模型", scale=3
                    )
                with gr.Row():
                    input_path = gr.Textbox(value="", label="批量处理路径(仅在本程序运行的电脑生效)", scale=5)
                    open_button = gr.Radio([True, False], value=False, label="是否启用批处理", scale=1)
            ...
            with gr.Column():
                with gr.Row():
                    realsr_model = gr.Radio(
                        ["models-DF2K_JPEG", "models-DF2K"], value="models-DF2K_JPEG", label="超分模型"
                    )
                    tta = gr.Radio([True, False], value=True, label="是否开启tta模式")
                with gr.Row():
                    input_path = gr.Textbox(value="", label="批量处理路径(仅在本程序运行的电脑生效)", scale=5)
